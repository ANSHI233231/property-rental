...
export const RENT_ACCRUAL_QUEUE = "rent-accrual";
export const RENT_ACCRUAL_JOB = "daily-accrual";

/**
 * RentAccrualProcessor — BullMQ worker for daily rent-period lifecycle management.
 *
 ...
 *
 * SYSTEM_JOB_USER is a sentinel actor ID for audit entries written by the cron.
 */
export const SYSTEM_JOB_ACTOR = "SYSTEM_ACCRUAL_JOB";

@Processor(RENT_ACCRUAL_QUEUE)
export class RentAccrualProcessor extends WorkerHost {
  private readonly logger = new Logger(RentAccrualProcessor.name);
  ...
  async process(job: Job): Promise<void> {
    this.logger.log(`Processing rent-accrual job ${job.id}`);
    await this.runAccrual();
  }

  @OnWorkerEvent("completed")
  onCompleted(job: Job) {
    this.logger.log(`Rent accrual job ${job.id} completed`);
  }

  @OnWorkerEvent("failed")
  onFailed(job: Job, err: Error) {
    this.logger.error(`Rent accrual job ${job.id} failed: ${err.message}`, err.stack);
  }
  ...
  async runAccrual(nowOverride?: Date): Promise<{
    date: string;
