/**
 * Unit tests for RentAccrualService — BL-12 / BL-13 logic.
 *
...
// ---------------------------------------------------------------------------
// RentAccrualService — service logic tests
// ---------------------------------------------------------------------------

describe("RentAccrualService", () => {
  let service: RentAccrualService;
  ...
  // ---------------------------------------------------------------------------
  // RUN_SCHEDULER=false — cron skips but runAccrual() still works
  // ---------------------------------------------------------------------------

  it("runDailyAccrual skips when RUN_SCHEDULER=false", async () => {
    (configMock.get as jest.Mock).mockReturnValue("false");
    const result = await service.runDailyAccrual();
    expect(result.skipped).toBe(true);
    expect(prismaMock.rentAccrualLog!.findUnique).not.toHaveBeenCalled();
  });
});
