...
/**
 * Admin Profile — Phase 6 (refactored: unified two-card layout).
 *
...
export default function AdminProfilePage() {
  ...
  const avatarBg = "#1A237E"; // Navy for admin
  ...
  return (
    <>
            ...
            <span className="value">
              {editingPhone ? (
                <form
                  ...
                  noValidate
                >
                  <Field id="admin-phone" label="" error={profileErrors.phone?.message}>
                    <input
                      type="tel"
                      className="input"
      ...
      <EditDetailsModal
        ...
        profile={
          profile
            ? { id: profile.id, name: profile.name, email: profile.email, phone: profile.phone ?? null }
            : null
        }
        allowEmailEdit
        onSuccess={(u) => {
          setProfile((prev) =>
            prev ? { ...prev, name: u.name, email: u.email, phone: u.phone } : prev,
          );
          setProfileSaveSuccess(true);
        }}
      />
