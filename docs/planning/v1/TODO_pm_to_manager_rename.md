...
| **T3** | Update [`SRS_Document.md` §3 page map](../../product/SRS_Document.md#3-pages--navigation-map) | All `prototype/pm/*.html` paths → `prototype/manager/*.html` |
| **T4** | Update [`Test_Cases.md`](../../testing/v1/Test_Cases.md) | Any test case that cites `pm/` file paths or URL routes |
...
| **T7** | Update [`docs/product/v1/GharSetu_Model_API_Spec.md`](../../product/v1/GharSetu_Model_API_Spec.md) | Probably none (the spec uses `MANAGER` enum, not URL paths) — worth a grep to confirm |
...
2. **Run a single sed sweep across docs:**
   ...
   grep -rl "prototype/pm/\|\bpm/dashboard\|\bpm/tenants\|\bpm/leases\|\bpm/rent-collection\|\bpm/maintenance\|\bpm/profile" \
     prototype/ docs/product/SRS_Document.md docs/testing/v1/Test_Cases.md AGENTS.md CLAUDE.md docs/product/v1/ \
...
5. **Re-run [`Test_Cases.md`](../../testing/v1/Test_Cases.md) §11 (Navigation & Linking) tests** — TC-NAV-001 through TC-NAV-010.
