...
**Last updated:** 2026-06-02 (cont.) · **Rent-collection period-first build (ALL ROLES) + Security-portal fixes (uncommitted).** Built a single-source rent engine (`assets/rent-engine.js` + `rent-data.js` + `rent-settings.js`) and reworked Record Payment to **period-first** across admin + PM (new `pm/record-payment.html`) + tenant listing. **Late fee = Settings-driven, month-end-capped (BL-13 CHANGED — see decisions).** Refinements: cumulative-due default, allocation Remaining+Total/Settled, no-default method, BL-code removal, "Monthly Rent" rename, "Collected this month", "Partial Paid" rename, Overdue-filter=tile, lease-type on rows, accrued-fee column, By-Building "See leases" popup + current-month badge + this-month Collected, **Portfolio tab removed**. **Security portal:** profile role fixed (PM→Security Guard), single-source `assets/security-visitors.js` (dashboard + visitors can't diverge), gate card removed. Sidebar labels made consistent per role. Orchestrator-direct + one `gharsetu-frontend` dispatch (the 4 role pages).

> **This session is prototype + docs only — NOT committed.** The prior session committed `ea42b73` (admin/super-admin polish + nav reorder + lease-detail security deposit) — summarized in §7. Everything since (rent period-first all-roles + security fixes) is the uncommitted working set; change-log `gharsetu-frontend-2026-06-02.md` (Tasks 1–10) + `prototype-changes.md` (2026-06-02 "period-first model BUILT" + "refinements" + "Security portal" rows) carry the detail.
> **BL-13 month-end-cap doc sync ✅ DONE** — SRS §5 (BL-13 row + Module 5 late-fee walkthrough + engineering "Don't") + the Settings "Late fee" copy are all re-synced to the capped rule in this working set (Working rule #9). Solution Overview docx regeneration is the only remaining doc touch if its copy references the cap.
...
## 2. Last session summary (2026-06-02 cont. — rent-collection period-first build, ALL ROLES + Security-portal fixes)

**Scope.** Opus orchestrator-direct + one `gharsetu-frontend` dispatch (the 4 role pages). All work in `prototype/` + `docs/`. **No app code. NOT committed.** Reference: user-supplied `docs/planning/RENT_COLLECTION_CONTEXT.md`.

**Shipped (prototype + docs):**
1. **Single-source rent engine** — NEW `assets/rent-engine.js` (oldest-first allocator, **month-end-capped** late fee, query-time status + arrears-aware `displayStatus`, unsettled periods, `cumulativeDueThrough`, `collectedThisMonth`, `feeOutstandingTotal`, transaction grouping) + NEW `assets/rent-data.js` (canonical 8-lease roster, numeric ids, both lease types, every status) + NEW `assets/rent-settings.js` (late-fee rate + grace, persisted; **fetched by the engine; edited on Settings page**). Verified by Node harness (status spread, allocation, 8% cap, doc fee examples).
2. **Record Payment → period-first** (admin + NEW `pm/record-payment.html`) — lease → 4 context cards → Billing-Period dropdown (pre-fills **cumulative due through the selected month**, editable) → Amount/Method → live **allocation preview** (Period · Rent · Fee · Status · **Remaining** + **Total** row; **Settled**/Partial) → sticky summary → grouped history. No default payment method (+ validation). Record-only.
3. **Listings reworked to the engine** (admin `rent.html`, PM `rent-collection.html`, tenant `rent.html`) — scope tiles (Monthly Rent · **Collected this month** · Outstanding · Overdue), By-Lease + By-Building, combined Property·Unit·Room cell, **lease type on each row**, **Late Fee column = total accrued fee**, **"Partial Paid"** (was Partial Overdue), Overdue filter = tile (incl. Partial Paid). **By-Building** "See leases" **popup** (was inline) + **current-month badge** + this-month Collected + collection-rate fix (was negative). **Portfolio tab removed** (admin mirrors PM).
4. **Late fee = Settings-driven + month-end-capped** — **BL-13 CHANGED** (was uncapped) per user decision; rate/grace read live from `rent-settings.js`; `effective=min(payDate, monthEnd)` ⇒ ~8%/period max. **SRS §5 + Settings copy re-sync PENDING.**
5. **Client-facing cleanup** — removed the Record-Payment audit footnote; stripped all `(BL-…)` codes from visible copy (12 refs / 6 pages); "Amount Due/Total Due" → **"Monthly Rent"** everywhere.
6. **Security portal fixes** — `security/profile.html` corrected from a leftover PM identity → **Security Guard** (Ramesh Gupta · gates · shift · gate activity); NEW `assets/security-visitors.js` **single source** → `security/dashboard.html` KPIs + awaiting list and `security/visitors.html` table both render from it (can't diverge); fixed a pre-existing off-by-one in the visitor row-action cells; removed the "Visitor at the gate?" card.
7. **Sidebar label consistency** — audited per role: admin "Rent collection" ×23, PM "Rent Collection" ×14, tenant fixed the lone outlier ("Rent & Payments" → "Rent", now ×8).

**Docs:** `gharsetu-frontend-2026-06-02.md` (Tasks 1–10); `prototype-changes.md` (period-first BUILT + refinements + Security rows); plan `2026-06-02-record-payment-period-first.md` updated (in-progress, build logged).

**Carry-over / pending:** (a) **BL-13 month-end-cap doc sync ✅ done** in SRS §5 + Settings copy (Solution Overview docx regen only if its copy cites the cap). (b) Open product calls flagged to user: tenant "Total Paid" lifetime vs staff "Collected this month"; admin vs PM "Rent collection/Collection" casing. (c) App port to `apps/web`/`apps/api` still pending. No blockers.
...
| Stream | Status |
|---|---|
| **Rent collection — period-first model (ALL ROLES)** on single-source engine (`rent-engine`/`rent-data`/`rent-settings`); admin+PM Record Payment, admin/PM/tenant listings, By-Building popup | ✅ shipped 2026-06-02 (uncommitted) |
| **BL-13 → month-end-capped + Settings-driven late fee** | ✅ shipped 2026-06-02 (uncommitted) · SRS §5 + Settings copy synced |
| **Security portal** — profile role fix + single-source `security-visitors.js` (dashboard+visitors) + gate card removed | ✅ shipped 2026-06-02 (uncommitted) |
...
| **Delegations Detail page** (`delegation-detail.html`; drawer + global `delegation-activity.html` removed) | ✅ shipped 2026-06-02 (uncommitted) |
| **Check Availability** page (filter rail + cards + popup → Create Lease; moved to Leases) | ✅ shipped 2026-06-02 (uncommitted) |
| **Rent collection completion** (query-time status model + view tabs + tiles) | ✅ shipped 2026-06-02 (uncommitted) |
| **Super-Admin reconcile** (dashboard/invoices/plans to real org set; month+org filters fixed) | ✅ shipped 2026-06-02 (uncommitted) |
| **Master Add/Edit bug fix (9 masters)** + Payment-Methods "Reference required" + Visit-Purposes "Requires details" + Business-Types "Other" removed | ✅ shipped 2026-06-02 (uncommitted) |
| **Admin nav reorder by priority** (sidebar + more-sheet, 28 pages; Audit Log → end) | ✅ shipped 2026-06-02 (uncommitted) |
| **Lease-detail Security Deposit** (refunds + Record-Refund modal, gated to ended/terminated; Schedule\|Deposit 6:6; Tenants merged into Summary card) | ✅ shipped 2026-06-02 (uncommitted) |
| **Settings trim** (same-page save, button redesign) · **Organization property-cap** + plan badge · invoice GST line removed · **renew deep-link fix** | ✅ shipped 2026-06-02 (uncommitted) |
| **Record Payment — period-first model** | 🟡 PLANNED only (`2026-06-02-record-payment-period-first.md`, BL-13 no cap) — 2 sign-off questions open, no code |
...
0. **Product calls flagged to user** (small): tenant "Total Paid" lifetime vs "this month"; admin/PM "Rent collection"/"Rent Collection" casing. (BL-13 month-end-cap SRS §5 + Settings copy already synced; regenerate Solution Overview docx only if its copy cites the cap.)
...
8. **Record Payment — period-first build** ✅ **DONE 2026-06-02** (all roles, on the single-source engine; both sign-off questions resolved by `RENT_COLLECTION_CONTEXT.md`). Remaining tail = the BL-13 SRS sync in item 0.
9. **Open prototype follow-ups (deferred this session):** (a) super-admin `organization-detail.html` Pending-org coherence — Saffron shows an *activated* plan history + 18 active users while Pending; (b) wire consuming visitor forms to reveal a detail input when a visit purpose has **Requires details**.
...
|---|---|---|---|
| 2026-06-02 (cont.) | _uncommitted_ | orchestrator (Opus) + gharsetu-frontend | **Rent-collection period-first build (ALL ROLES) + Security-portal fixes.** NEW single-source engine (`rent-engine.js` + `rent-data.js` + `rent-settings.js`); Record Payment → period-first (admin + NEW `pm/record-payment.html`); admin/PM/tenant listings reworked to the engine (Monthly Rent · Collected this month · Outstanding · Overdue; By-Lease + By-Building popup + current-month badge; lease-type rows; accrued-fee column; "Partial Paid"; Overdue filter=tile; **Portfolio tab removed**). **BL-13 CHANGED → month-end-capped, Settings-driven** (SRS/Settings re-sync PENDING). `(BL-…)` codes + audit footnote stripped; "Amount Due/Total Due" → "Monthly Rent". **Security:** profile PM→Security-Guard, single-source `security-visitors.js` (dashboard+visitors), gate card removed; sidebar labels consistent per role. Change-log `gharsetu-frontend-2026-06-02.md` Tasks 1–10 + 3 ledger rows. |
| 2026-06-02 | `ea42b73` | orchestrator (Opus) + gharsetu-frontend | **Admin/Super-Admin polish + nav reorder + lease-detail Security Deposit + Record-Payment plan.** Delegation Detail page (drawer + `delegation-activity.html` removed) · Check Availability built + moved to Leases · Rent-collection query-time status model · Super-Admin dashboard/invoices/plans reconciled to real orgs + month/org filters fixed · Master Add/Edit bug fixed across 9 masters + Payment-Methods "Reference required" + Visit-Purposes "Requires details" + Business-Types "Other" removed · **admin nav reordered by priority on 28 pages** (Audit Log → end; one-off `scripts/` transform since deleted) · **Lease-detail Security Deposit** section + refund modal (gated ended/terminated) + Schedule\|Deposit 6:6 + Tenants merged into Summary card · Settings trim + same-page save + button redesign · Organization property-cap + plan badge · renew deep-link fix (`2114`) · invoice GST line removed · **Record Payment period-first model PLANNED only** (BL-13 no cap, 14 TCs, 2 sign-off Qs open). Change-log Tasks 10–23 + ~25 ledger rows. |
| 2026-06-01 | `adc1b7d`·`1ffaea1`·`d8cbc16`·`bac96b5` | orchestrator (Opus) + gharsetu-frontend | **Admin lease/unit feature pass → Security Guard role → public-pages v2 → rent-collection overhaul** (four commits). Auth email-only · org-detail multi-business-type + Primary Contact · lease-detail Renew + Early-Termination consent · **PM property/lease clone** (`pm/create-lease.html`) · lease status labels standardized · prototype listing enhancements (`Paginator.setPredicate`, clickable KPIs) · **NEW Security Guard role (role 6)** + gate-initiated visitor flow · public-pages redesign v2 (glass/light) · **rent-collection overhaul** (full-page Record Payment + late-fee allocation, view tabs) + **Leases↔Rent↔Lease-detail reconciliation** (canonical 24-lease roster). SRS synced (6 roles, Module 8). |
